...
# lunix support
if (-not $env:VIRTUALENVWRAPPER_DIR) {
    $global:VIRTUALENVWRAPPER_DIR = "Scripts"
    if ($PSVersionTable.Platform -like "unix") {
        $global:VIRTUALENVWRAPPER_DIR = "bin"
    }
}

# locate the global python where virtualenvwrapper is installed
if (-not $VIRTUALENVWRAPPER_PYTHON) {
    $global:VIRTUALENVWRAPPER_PYTHON = @(get-command python)[0].definition
}
...
function New-VirtualEnvironment
{
    ...
    push-location $env:WORKON_HOME
        & $global:VIRTUALENVWRAPPER_VIRTUALENV $virtualenv_args $Name $global:VIRTUALENVWRAPPER_VIRTUALENV_ARGS
    pop-location
...
#>
{
    ...
    $activate = get-item "$env:WORKON_HOME/$env_name/$global:VIRTUALENVWRAPPER_DIR/activate.ps1" -errora silentlycontinue
    ...
    if (test-path $env:WORKON_HOME/$env_name/.project) {
        $folder = Get-Content $env:WORKON_HOME/$env_name/.project;
        cd $folder;
    }
